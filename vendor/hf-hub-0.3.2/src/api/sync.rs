...
impl Api {
    ...
    fn metadata(&self, url: &str) -> Result<Metadata, ApiError> {
        ...
        let response = if is_redirection {
            let redirect_loc = response.header(LOCATION).unwrap();
            let final_url = if redirect_loc.starts_with("http://") || redirect_loc.starts_with("https://") {
                redirect_loc.to_string()
            } else if let Some(pos) = url.find("://") {
                let rest = &url[pos + 3..];
                let slash_pos = rest.find('/').unwrap_or(rest.len());
                let base = &url[..pos + 3 + slash_pos];
                if redirect_loc.starts_with('/') {
                    format!("{}{}", base, redirect_loc)
                } else {
                    format!("{}/{}", base, redirect_loc)
                }
            } else {
