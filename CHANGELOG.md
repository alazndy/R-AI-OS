...
- **Coverage floor raised 40% → 42%:** added tests across CLI, TUI, MCP, and a new `new_project::create` integration test, raising the enforced line-coverage floor from 40% to 42%.
- **Panic-vector hardening:** removed 10 real panic vectors found during a full manual audit of every `raios refactor`-flagged file, including `daemon/handlers.rs`'s inconsistent unwrap-vs-safe-serialize pattern in the WS connection handler, `hub.rs`'s `probe_port` IPv6 `SocketAddr` construction bug and `cmd_logs`'s tail-binary crash, `intelligence/instinct.rs`'s `dirs::home_dir()` panic reachable from every `raios run`/`raios task` call, and hardened `agent_runner.rs`'s `wrapper_note_ipc` test against transient CI connection resets.
- **`raios-tray` audit:** full security, dependency, and quoting audit of `tools/raios-tray/raios-tray.py` (1909 lines, previously untouched by tooling), fixing a shell-quoting design flaw in `open_terminal()` affecting macOS/Windows agent-launch paths, canonicalizing a live systemd service (`raios-tray.service`) that had drifted to a deprecated venv path, adding `requirements.lock.txt` for reproducible dependency installs (matching `Cargo.lock`/`pnpm-lock.yaml`), and adding logging to 9 previously-silent exception handlers.
- **VS Code extension dependency fix:** resolved a HIGH-severity `brace-expansion` DoS vulnerability (GHSA-mh99-v99m-4gvg) in transitive dependencies of `@vscode/vsce` via a `pnpm-workspace.yaml` override pinning `brace-expansion` to `5.0.8`.
- **System map regenerated:** regenerated `SIGMAP.md` to reflect recent codebase stabilization and structural updates.
