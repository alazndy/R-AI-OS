...
pub fn render_quick_stats(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let rf_high_total: usize = app.health_report.iter().map(|h| h.refactor_high_count).sum();
    let rf_projects_with_issues: Vec<&str> = app.health_report.iter()
        .filter(|h| h.refactor_high_count > 0)
        .map(|h| h.name.as_str())
        .take(3)
        .collect();
    let rf_projects_total = app.health_report.iter().filter(|h| h.refactor_high_count > 0).count();
    ...
    let lines = if app.is_computing_stats {
        ...
        let mut result = vec![
            Line::from(vec![
                ...
                Span::styled(format!("  {} no-sig", s.no_sigmap), Style::new().fg(if s.no_sigmap > 0 { RED } else { DIM })),
            ]),
            Line::from(vec![
                Span::styled(format!("  A:{:>3}% ", grade_pct(s.grade_a as usize)), Style::new().fg(GREEN)),
                Span::styled(format!("B:{:>3}% ", grade_pct(s.grade_b as usize)), Style::new().fg(CYAN)),
                Span::styled(format!("C:{:>3}% ", grade_pct(s.grade_c as usize)), Style::new().fg(AMBER)),
                Span::styled(format!("D:{:>3}%", grade_pct(s.grade_d as usize)), Style::new().fg(RED)),
            ]),
            Line::from(vec![
                Span::styled(format!("  local-only:{}", s.no_github), Style::new().fg(DIM)),
            ]),
        ];
        ...
        if rf_high_total > 0 {
            ...
            result.push(Line::from(Span::styled(
                format!("  ⚠ REFACTOR: {} HIGH files across {} projects",
