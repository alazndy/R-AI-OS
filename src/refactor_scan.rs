...
impl RefactorReport {
    pub fn empty() -> Self {
        Self { issues: vec![], score: 100, grade: "A".into(), high_count: 0, medium_count: 0 }
    }
}

// ─── Constants ───────────────────────────────────────────────────────────────

const SOURCE_EXTS: &[&str] = &["rs", "ts", "tsx", "js", "jsx", "py", "kt", "swift", "go", "java", "cpp", "c"];
...
pub fn scan_project(root: &Path) -> RefactorReport {
    ...
    let mut issues: Vec<RefactorIssue> = files.iter()
        .filter_map(|f| analyze_file(f))
        .collect();

    let high_count   = issues.iter().filter(|i| i.severity == RefactorSeverity::High).count();
    let medium_count = issues.iter().filter(|i| i.severity == RefactorSeverity::Medium).count();
    ...
    RefactorReport { issues, score, grade, high_count, medium_count }
}
...
fn analyze_file(path: &Path) -> Option<RefactorIssue> {
    ...
    if lines >= HIGH_LINE_THRESHOLD {
        ...
        reasons.push(format!("{} lines (>{} limit)", lines, MEDIUM_LINE_THRESHOLD));
    }
...
fn determine_severity(lines: usize, unwrap_count: usize, max_depth: usize) -> RefactorSeverity {
    let is_high = lines >= HIGH_LINE_THRESHOLD
        || unwrap_count >= UNWRAP_HIGH
        || max_depth >= 6;

    let is_medium = lines >= MEDIUM_LINE_THRESHOLD
        || unwrap_count >= UNWRAP_MEDIUM
        || max_depth >= 5;

    if is_high   { RefactorSeverity::High }
    else if is_medium { RefactorSeverity::Medium }
    else         { RefactorSeverity::Low }
}
...
fn collect_recursive(dir: &Path, depth: usize, files: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if matches!(name.as_ref(), "." | ".." | "target" | "node_modules" | "dist" | "__pycache__" | ".git")
            || name.starts_with('.')
        {
            continue;
        }
        ...
        } else if path.extension().and_then(|e| e.to_str()).map(|e| SOURCE_EXTS.contains(&e)).unwrap_or(false) {
            files.push(path);
        }
    }
}
