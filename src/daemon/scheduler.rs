...
pub async fn start_scheduler_worker(
    ...
    if let Ok(conn) = crate::db::open_db() {
        if let Err(e) = crate::db::cp_scheduled_jobs_reset_firing(&conn) {
    ...
    loop {
        ...
        let jobs = match crate::db::cp_scheduled_jobs_claim_due(&conn) {
            Ok(j) => j,
            Err(e) => {
        ...
        for job in jobs {
            ...
            tokio::spawn(async move {
                ...
                match spawn_result {
                    Ok(Ok(pid)) => {
                        let _ = crate::db::cp_scheduled_job_mark_fired(&conn, &job_id, interval);
                    ...
                    _ => {
                        let _ = crate::db::cp_scheduled_job_revert_firing(&conn, &job_id);
