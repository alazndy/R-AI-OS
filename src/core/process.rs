...
fn list_ports_windows() -> Vec<PortEntry> {
    ...
    for line in out.lines() {
        ...
        let port: Option<u16> = cols[1].split(':').last().and_then(|p| p.parse().ok());
        ...
        entries.push(PortEntry { port, pid, process_name, state, protocol: proto.to_string() });
    }
...
fn process_name_windows(pid: u32) -> Option<String> {
    let out = Command::new("tasklist")
        ...
        .map(|o| String::from_utf8_lossy(&o.stdout).into_owned())
        .unwrap_or_default();
    out.lines().next()?.split(',').next().map(|s| s.trim_matches('"').to_string())
}
...
fn list_ports_unix() -> Vec<PortEntry> {
    ...
    for line in out.lines().skip(1) {
        let cols: Vec<&str> = line.split_whitespace().collect();
        if cols.len() < 4 { continue; }
        let port: Option<u16> = cols[3].split(':').last().and_then(|p| p.parse().ok());
        let port = match port { Some(p) => p, None => continue };
        let (pid, name) = parse_ss_process(cols.get(6).copied().unwrap_or(""));
        entries.push(PortEntry { port, pid, process_name: name, state: "LISTEN".into(), protocol: "TCP".into() });
    }
    entries.sort_by_key(|e| e.port);
    entries
}
...
fn parse_ss_process(s: &str) -> (Option<u32>, Option<String>) {
    let pid  = s.split("pid=").nth(1).and_then(|p| p.split(',').next()).and_then(|p| p.parse().ok());
    let name = s.split("((\"").nth(1).and_then(|p| p.split('"').next()).map(str::to_string);
    (pid, name)
}
...
fn list_processes_windows(top_n: usize) -> Vec<ProcessEntry> {
    ...
    let mut entries: Vec<ProcessEntry> = out.lines().filter_map(|line| {
        let cols: Vec<&str> = line.splitn(5, ',').collect();
        if cols.len() < 5 { return None; }
        let name = cols[0].trim_matches('"').to_string();
        let pid: u32 = cols[1].trim_matches('"').parse().ok()?;
        let mem_kb: f32 = cols[4].trim_matches('"').replace(',', "").replace(" K", "").trim().parse().unwrap_or(0.0);
        Some(ProcessEntry { pid, name, cpu_pct: None, mem_mb: Some(mem_kb / 1024.0) })
    }).collect();

    entries.sort_by(|a, b| b.mem_mb.unwrap_or(0.0).partial_cmp(&a.mem_mb.unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));
...
fn list_processes_unix(top_n: usize) -> Vec<ProcessEntry> {
    ...
    out.lines().skip(1).take(top_n).filter_map(|line| {
        let cols: Vec<&str> = line.split_whitespace().collect();
        ...
        let name = cols[10].split('/').last().unwrap_or(cols[10]).to_string();
        Some(ProcessEntry { pid, name, cpu_pct: Some(cpu), mem_mb: Some(mem) })
    }).collect()
}
...
pub fn kill_port(port: u16) -> KillResult {
    match list_ports().iter().find(|e| e.port == port) {
        None    => KillResult { ok: false, message: format!("No process listening on port {}", port) },
        Some(e) => match e.pid {
            None      => KillResult { ok: false, message: format!("Port {} found but PID unknown", port) },
...
pub fn kill_pid(pid: u32) -> KillResult {
    ...
    match out {
        Ok(o) if o.status.success() => KillResult { ok: true, message: format!("Process {} terminated", pid) },
        Ok(o) => KillResult { ok: false, message: String::from_utf8_lossy(&o.stderr).trim().to_string() },
        Err(e) => KillResult { ok: false, message: e.to_string() },
    }
}
...
mod tests {
    ...
    fn list_ports_no_panic() {
        ...
        for p in &ports { assert!(p.port > 0); }
    }
    ...
    fn list_processes_bounded() {
        ...
        assert!(procs.len() <= 5);
        for p in &procs { assert!(p.pid > 0); }
    }

    #[test]
    fn kill_nonexistent_port_returns_error() {
        let r = kill_port(1);
        if !r.ok { assert!(!r.message.is_empty()); }
    }
