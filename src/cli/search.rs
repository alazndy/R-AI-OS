...
pub(super) fn cmd_search(query: &str, top_k: usize, reindex: bool, dev_ops: &Path, json: bool) {
    ...
    if needs_index {
        if !json {
            if reindex { println!("Cortex: Re-indexing workspace (forced)..."); }
            else { println!("Cortex: First run — building index..."); }
        }
        let indexed = cortex.index_workspace(dev_ops).unwrap_or(0);
        if !json { println!("Indexed {} chunks. Searching...\n", indexed); }
    } else if !json {
    ...
    if json {
        let results: Vec<serde_json::Value> = fused.iter().map(|r| serde_json::json!({
    ...
    println!("{}", "─".repeat(72));
    if fused.is_empty() { println!("No results found."); return; }
...
pub(super) fn cmd_cortex_index(force: bool, dev_ops: &Path, json: bool) {
    let mut cortex = match crate::cortex::Cortex::init() {
        Ok(c) => c,
        Err(e) => { eprintln!("Cortex init failed: {e}"); std::process::exit(1); }
    };
    ...
    if !json { println!("Indexing workspace… (this may take a minute on first run)"); }
    match cortex.index_workspace(dev_ops) {
        Ok(n) => {
            if json { println!("{}", serde_json::json!({"status":"ok","indexed":n,"total_chunks":cortex.chunk_count()})); }
            else { println!("✓ Indexed {} files ({} chunks total)", n, cortex.chunk_count()); }
        }
