use super::*;
use crate::security::{render_rules_toml, suggest_policy_rules, PolicyConfig};
...
fn cmd_simulate(tool: String, args: Option<String>, json: bool) {
    ...
    let caps = crate::security::capabilities::resolve(&tool, umai.tool_capabilities(&tool));
    let is_path_resolving = crate::security::capabilities::PATH_RESOLVING_TOOLS.contains(&tool.as_str());

    let (decision_label, reason): (&str, Option<String>) = match &decision {
        crate::security::UmaiDecision::Allow => ("allow", None),
        crate::security::UmaiDecision::Deny(r) => ("deny", Some(r.clone())),
        crate::security::UmaiDecision::Confirm(r) => ("confirm", Some(r.clone())),
    };
...
fn cmd_caps(tool: Option<String>, json: bool) {
    ...
    let tools: Vec<String> = match &tool {
        Some(t) => vec![t.clone()],
        None => crate::security::capabilities::ALL_TOOLS.iter().map(|s| s.to_string()).collect(),
    };
    ...
    if json {
        let out: Vec<_> = tools
            ...
            .map(|t| {
                ...
                let caps = crate::security::capabilities::resolve(t, override_caps.clone());
                serde_json::json!({
                    "tool": t,
                    "fs_read": caps.fs_read,
                    "fs_write": caps.fs_write,
    ...
    for t in &tools {
        ...
        let caps = crate::security::capabilities::resolve(t, override_caps);
        println!(
...
fn print_suggestions_human(s: &crate::security::PolicySuggestions, min_count: usize) {
...
fn apply_suggestions(s: &crate::security::PolicySuggestions, json: bool) {
