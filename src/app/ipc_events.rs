...
fn handle_state_sync(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    ...
    let pending_file_changes =
        serde_json::from_value::<Vec<crate::daemon::state::FileChangeApproval>>(
            v["pending_file_changes"].clone(),
        )
        .unwrap_or_default();
    let sentinel_files =
        serde_json::from_value::<Vec<crate::daemon::state::SentinelFileStatus>>(
...
fn handle_file_change_requested(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    if let Ok(approval) =
        serde_json::from_value::<crate::daemon::state::FileChangeApproval>(v["approval"].clone())
    {
