...
pub fn connect_daemon_addr(tx: Sender<BgMsg>, remote_host: Option<String>) -> Option<Sender<String>> {
    let (daemon_addr, is_remote) =
        crate::app::ipc_support::resolve_daemon_addr(remote_host.as_deref());
    ...
    thread::spawn(move || {
        ...
        if !is_remote {
            crate::app::ipc_support::ensure_local_daemon_running();
        }
        ...
        loop {
            match std::net::TcpStream::connect(&daemon_addr) {
                Ok(mut stream) => {
                    crate::app::ipc_support::write_auth_handshake(&mut stream, is_remote);
                    ...
                    crate::app::ipc_support::initial_state_request(&mut stream);

                    // Notify TUI that daemon is now connected
                    tx.send(BgMsg::NewLog(crate::app::ipc_support::log_entry(
                        "IPC",
                    ...
                    let reader_handle = thread::spawn(move || {
                        ...
                        for line in reader.lines().map_while(|r| r.ok()) {
                            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&line) {
                                crate::app::ipc_events::dispatch_event(&tx_read, &v);
                            }
                        }
                    });
            ...
            thread::sleep(crate::app::ipc_support::RETRY_INTERVAL);
        }
    });
