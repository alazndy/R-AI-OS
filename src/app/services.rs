...
use crate::db::{RunOverviewRow, ScoredApproval, ScheduledJob, UnifiedTaskRow};
use crate::{entities::EntityProject, health::ProjectHealth};
...
pub struct PoliciesPanelData {
    pub policy: Option<crate::security::PolicyConfig>,
...
fn load_inbox_panel_data_from_conn(
    ...
    Ok(InboxPanelData {
        approvals: crate::db::cp_query_pending_approvals_scored(conn)?,
        runs: crate::db::cp_query_active_runs(conn)?,
        blocked: crate::db::cp_query_blocked_tasks(conn)?,
    })
}
...
pub fn load_project_detail_data(project_path: &Path) -> ProjectDetailData {
    ...
    let content = crate::filebrowser::load_file_content(&memory_path);
    ProjectDetailData {
        memory_lines: content.lines().map(str::to_owned).collect(),
        git_log: crate::filebrowser::get_git_log(project_path),
    }
}
...
pub fn load_graph_report_lines(project_path: &Path) -> Result<Vec<String>, String> {
    ...
    Ok(crate::filebrowser::load_file_content(&report_path)
        .lines()
        .map(str::to_owned)
        .collect())
}
...
mod tests {
    ...
    fn inbox_panel_data_defaults_to_empty() {
        ...
        crate::db::migrate_existing(&conn).unwrap();
