...
impl McpServer {
    ...
    pub(super) fn tool_list_swarm_tasks(&self) -> Result<Value, String> {
        ...
        let text = if tasks.is_empty() { "No active swarm tasks.".to_string() }
        else { tasks.iter().map(|t| format!("{} [{}] {} — {}", t.id, format!("{:?}", t.status).to_lowercase(), t.agent, t.task_description)).collect::<Vec<_>>().join("\n") };
            ...
            Some(task) => {
                ...
                match crate::swarm::merge::merge_branch(&task.project_path, &task.branch_name, &msg) {
                    Ok(_) => {
                        ...
                        Ok(json!({ "content": [{ "type": "text", "text": format!("✓ Merged: {task_id}") }], "merged": true }))
                    }
    ...
    pub(super) fn tool_route_capability(&self, args: &Value) -> Result<Value, String> {
        ...
        let router = crate::intelligence::edge::EdgeRouter::new(descriptions);
        let capability = router.route(query);
        let text = match capability { Some(c) => format!("→ {c}"), None => format!("No matching capability for: {query}") };
    ...
    pub(super) fn tool_list_evolution_candidates(&self, args: &Value) -> Result<Value, String> {
        ...
        let store = crate::intelligence::evolution::CandidateStore::new(
            crate::intelligence::evolution::CandidateStore::default_path()
        );
        let candidates = store.list_pending(limit);
        let text = if candidates.is_empty() { "No pending instinct candidates.".to_string() }
        else { candidates.iter().enumerate().map(|(i, r)| format!("{}. {}", i + 1, r)).collect::<Vec<_>>().join("\n") };
    ...
    pub(super) fn tool_promote_evolution_candidate(&self, args: &Value) -> Result<Value, String> {
        ...
        let _ = crate::intelligence::instinct::append_to_memory_md(&cwd, rule);
        Ok(json!({ "content": [{ "type": "text", "text": format!("✓ Promoted: {rule}") }], "promoted": rule }))
    }
}
