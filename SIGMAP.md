...
<!-- Updated: 2026-08-10T06:59:28.705Z -->
...
```
crates/raios-surface-mcp/src/mcp/tools.rs     +directly
```
...
```
pub fn estimate_tokens  :4-6
pub fn savings_pct  :9-16
```
...
pub fn run_doctor_check(agent: &str, requested_tier: Option<DoctorTier>) → DoctorResult  :78-219
```
...
pub struct App  :198-275
impl App  :277-281
impl App  :283-429
  pub fn new() → Self  :285-285
  pub fn new_remote(host: String) → Self  :416-416
pub fn filtered_palette(query: &str) → Vec<&'static PaletteItem>  :167-175
```

### crates/raios-surface-tui/src/app/reducer.rs
```
pub fn reduce_intent(store: &mut Store, intent: Intent)  :9-66
pub fn reduce_event(store: &mut Store, event: Event)  :69-104
```
...
```
pub struct Store  :24-57
pub enum WorkFocus  :12-20
impl Store  :59-87
impl Store  :89-132
  pub fn new() → Self  :91-91
  pub fn set_snapshot(&mut self, env: SnapshotEnvelope)  :96-96
  pub fn add_log(&mut self, log: impl Into<String>)  :101-101
  pub fn work_project_indices(&self) → Vec<usize>  :109-109
```
...
pub fn render_launcher(frame: &mut Frame, area: Rect, app: &App)  :112-156
pub fn render_command_palette(frame: &mut Frame, app: &App)  :159-221
pub fn render_file_changed_badge(frame: &mut Frame, _app: &App)  :224-240
pub fn render_launcher_modal(frame: &mut Frame, app: &App)  :243-294
pub fn center_rect(width: u16, height: u16, parent: Rect) → Rect  :297-306
pub fn render_handover_modal(frame: &mut Frame, app: &App)  :309-376
pub fn render_constitution_save_modal(frame: &mut Frame, app: &App)  :379-427
```
...
pub fn render(frame: &mut Frame, app: &App)  :67-107
pub fn update_code_block_state(line: &str, ext: &str, in_block: &mut bool)  :136-143
pub fn highlight_line(line: &'a str, in_block: &mut bool, ext: &str) → Vec<Span<'a>>  :146-166
```
...
```
pub fn render_work_route(f: &mut Frame, area: Rect, store: &Store)  :37-171
```
