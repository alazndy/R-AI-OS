...
use raios_core::task_graph::GraphStore;
use std::sync::Arc;
...
pub async fn handle_create_task_graph<W: AsyncWriteExt + Unpin>(
    ...
    let nodes: Vec<raios_core::task_graph::NodeSpec> = nodes_val
        .iter()
        .filter_map(|n| {
            Some(raios_core::task_graph::NodeSpec {
    ...
    loop {
        ...
        let ready_tasks: Vec<(String, String, String)> = if let Ok(conn) = raios_core::db::open_db()
        {
            raios_core::db::cp_scheduler_list_ready(&conn)
                ...
                .filter_map(|t| {
                    let cmd = raios_core::db::cp_task_graph_shell_cmd(&conn, &t.id)
                        .ok()
                        .flatten()?;
                    let (_, node_id) = raios_core::db::cp_task_graph_node_ids(&conn, &t.id)
                        .ok()
                        .flatten()?;
        ...
        for (_task_id, node_id, cmd) in ready_tasks {
            ...
            factory2.submit(
                job,
                Box::pin(async move {
                    let (program, args) = raios_core::core::process::shell_command(&cmd);
