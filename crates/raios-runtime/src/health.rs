...
pub fn check_project(proj: &EntityProject) -> ProjectHealth {
    ...
    let (ci_status, ci_url) = if remote_url.as_deref().unwrap_or("").contains("github.com") {
        match raios_core::core::ci::get_ci_status(path) {
            Ok(report) => {
...
pub fn check_build_test_deps(h: &mut ProjectHealth) {
    let build = raios_core::core::build::build(&h.path);
    h.build_ok = Some(build.ok);

    let test = raios_core::core::build::test(&h.path);
    h.test_passed = Some(test.passed);
    h.test_failed = Some(test.failed);

    let deps = raios_core::core::deps::check(&h.path);
...
pub fn check_project_with_security(proj: &EntityProject) -> ProjectHealth {
    ...
    let report = raios_core::security::scan_project(&h.path);
