...
pub(super) fn check_npm_tool(cmd: &str, name: &str) -> SystemAiTool {
    match raios_core::core::process::resolve_command_path(cmd) {
...
pub(super) fn check_antigravity() -> SystemAiTool {
    let path = raios_core::core::process::resolve_command_path("antigravity");
...
pub(super) fn check_opencode() -> SystemAiTool {
    match raios_core::core::process::resolve_command_path("opencode") {
        Some(path) => SystemAiTool {
            name: "OpenCode".into(),
            status: ToolStatus::Installed,
            version: Some("Active".into()),
...
fn cursor_candidates() -> Vec<PathBuf> {
    ...
    if let Some(path) = raios_core::core::process::resolve_command_path("cursor") {
        paths.push(path);
    }
...
fn lm_studio_candidates() -> Vec<PathBuf> {
    ...
    if let Some(path) = raios_core::core::process::resolve_command_path("lmstudio") {
        paths.push(path);
    }
    if let Some(path) = raios_core::core::process::resolve_command_path("lm-studio") {
        paths.push(path);
    }
