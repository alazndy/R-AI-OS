...
pub fn render_policies(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let data = raios_surface_tui::app::load_policies_panel_data();
    ...
    if let Some(ref p) = policy {
        let default_color = match p.tools.default_action {
            raios_core::security::policy::PolicyAction::Allow => GREEN,
            raios_core::security::policy::PolicyAction::Confirm => AMBER,
            raios_core::security::policy::PolicyAction::Deny => RED,
        };
        lines.push(Line::from(vec![
        ...
        let notable: Vec<_> = p
            ...
            .rules
            .iter()
            .filter(|r| r.action != raios_core::security::policy::PolicyAction::Allow)
            .take(4)
            .collect();
        for rule in &notable {
            let (tag, col) = match rule.action {
                raios_core::security::policy::PolicyAction::Deny => ("DENY   ", RED),
                raios_core::security::policy::PolicyAction::Confirm => ("CONFIRM", AMBER),
                raios_core::security::policy::PolicyAction::Allow => ("ALLOW  ", GREEN),
            };
            lines.push(Line::from(vec![
