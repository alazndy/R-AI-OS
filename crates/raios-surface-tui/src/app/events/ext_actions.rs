...
use raios_surface_tui::app::{state::BgMsg, App};
...
impl App {
    ...
    pub fn load_extensions(&mut self) {
        ...
        thread::spawn(move || {
            let exts = raios_surface_tui::app::scan_extensions(&dev_ops);
    ...
    pub fn save_ext_config_field(&mut self) {
        ...
        match raios_surface_tui::app::write_env_key(&env_path, &field.key, &new_val) {
            Ok(_) => {
