...
fn handle_health_report(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    if let Ok(r) =
        serde_json::from_value::<Vec<raios_runtime::health::ProjectHealth>>(v["report"].clone())
    {
...
fn handle_state_sync(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    let projects =
        serde_json::from_value::<Vec<raios_core::entities::EntityProject>>(v["projects"].clone())
            .unwrap_or_default();
    let health_reports = serde_json::from_value::<Vec<raios_runtime::health::ProjectHealth>>(
        v["health_reports"].clone(),
    )
    .unwrap_or_default();
    let active_agents = serde_json::from_value::<Vec<raios_runtime::daemon::proxy::AgentProcess>>(
    ...
    let pending_file_changes = serde_json::from_value::<
        Vec<raios_runtime::daemon::state::FileChangeApproval>,
    >(v["pending_file_changes"].clone())
    .unwrap_or_default();
    let sentinel_files = serde_json::from_value::<
        Vec<raios_runtime::daemon::state::SentinelFileStatus>,
...
fn handle_file_change_requested(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    if let Ok(approval) = serde_json::from_value::<raios_runtime::daemon::state::FileChangeApproval>(
        v["approval"].clone(),
    ) {
...
fn handle_new_log(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    if let Ok(log) =
        serde_json::from_value::<raios_surface_tui::app::state::LogEntry>(v["log"].clone())
    {
...
fn handle_umai_blocked(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    if let Some(reason) = v["reason"].as_str() {
        tx.send(BgMsg::NewLog(
            raios_surface_tui::app::ipc_support::log_entry("UMAI", format!("Blocked: {}", reason)),
        ))
        .ok();
    }
}
...
fn handle_job_complete(tx: &Sender<BgMsg>, v: &serde_json::Value) {
    ...
    if lines.is_empty() {
        ...
        for line in lines {
            tx.send(BgMsg::NewLog(
                raios_surface_tui::app::ipc_support::log_entry("RUN", line.to_string()),
            ))
            .ok();
        }
    }
