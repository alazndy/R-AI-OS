...
impl App {
    pub fn new() -> Self {
        ...
        if !config.dev_ops_path.as_os_str().is_empty() {
            let (runtime_tx, runtime_rx) = mpsc::channel::<raios_runtime::workers::RuntimeEvent>();
            ...
            raios_runtime::workers::spawn_embedded_workers(runtime_tx, config.dev_ops_path.clone());
        }
    ...
    pub fn run_graphify(&self, project_path: &Path) -> String {
        ...
        if raios_core::core::process::launch_in_terminal(&graphify_cmd, project_path) {
