...
pub fn cmd_security(
    ...
    use raios_core::security::{scan_project, Severity};
...
fn print_report(report: &raios_core::security::SecurityReport, json: bool) {
    if json {
...
fn print_guard_result(path: &Path, issues: &[raios_core::security::SecurityIssue], json: bool) {
    if json {
        if issues.is_empty() {
            return;
        }
...
fn send_toast(path: &Path, issues: &[raios_core::security::SecurityIssue]) {
