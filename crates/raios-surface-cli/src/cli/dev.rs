...
pub(super) fn cmd_disk(project: Option<String>, dev_ops: &Path, json: bool) {
    ...
    let reports = if project.is_none() {
        ...
        vec![disk::analyze(&super::resolve_project_path(
            project, dev_ops,
        ))]
    };
    if json {
...
pub(super) fn cmd_clean(
    ...
    let paths: Vec<std::path::PathBuf> = if all {
        raios_core::db::open_db()
            .and_then(|conn| raios_core::db::load_all_projects(&conn))
            .map(|ps| {
                ps.iter()
...
pub(super) fn cmd_usage(json: bool) {
    let report = raios_runtime::system_scan::scan_system();
    if json {
        println!(
            "{}",
            serde_json::to_string_pretty(&report.usage).unwrap_or_default()
        );
        return;
    }
    ...
    println!(
        "{:<20} {:<10} {:<8} {:<13} {:<14} {:<14}",
        "PROVIDER", "STATE", "AUTH", "PLAN", "REMAINING", "RESET"
    );
    println!("{}", "─".repeat(88));

    for usage in &report.usage {
        let state = if usage.installed {
            "installed"
        } else {
            "missing"
        };
        let auth = if usage.authenticated { "yes" } else { "no" };
        ...
        println!(
            "{:<20} {:<10} {:<8} {:<13} {:<14} {:<14}",
            usage.provider, state, auth, plan, remaining, reset_at
        );

        println!(
            "  kind:{}  renew:{}  auth_exp:{}  confidence:{:?}",
            usage.quota_kind,
            usage.renews_at.as_deref().unwrap_or("unknown"),
            usage.auth_expires_at.as_deref().unwrap_or("unknown"),
            usage.confidence
        );
...
pub(super) fn cmd_kill_port(port: u16, json: bool) {
    let r = raios_core::core::process::kill_port(port);
    if json {
...
fn print_deps_report(r: &raios_core::core::deps::DepsReport, json: bool) {
    if json {
        println!("{}", serde_json::to_string_pretty(r).unwrap_or_default());
        return;
    }
...
pub(super) fn cmd_build(
    ...
    use raios_core::core::build::{self, detect_type, ProjectType};
    let path = super::resolve_project_path(project, dev_ops);
...
pub(super) fn cmd_ci(project: Option<String>, dev_ops: &Path, json: bool) {
    use raios_core::core::ci::get_ci_status;
