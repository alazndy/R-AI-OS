...
impl McpServer {
    pub(super) fn tool_disk_usage(&self, args: &Value) -> Result<Value, String> {
        ...
        let r = raios_core::core::disk::analyze(&path);
        let mut lines = vec![format!(
            "Total: {}  Source: {}  Cache: {}  Files: {}",
            raios_core::core::disk::human_size(r.total_bytes),
            raios_core::core::disk::human_size(r.source_bytes),
            raios_core::core::disk::human_size(r.cache_bytes),
            r.file_count
        )];
        for c in &r.cache_dirs {
            lines.push(format!(
                ...
                raios_core::core::disk::human_size(c.bytes),
                c.kind
            ));
        }
        for (f, s) in r.largest_files.iter().take(5) {
            lines.push(format!(
                ...
                raios_core::core::disk::human_size(*s)
            ));
        }
        Ok(
    ...
    pub(super) fn tool_list_ports(&self) -> Result<Value, String> {
        let ports = raios_core::core::process::list_ports();
    ...
    pub(super) fn tool_usage_status(&self) -> Result<Value, String> {
        ...
        let text = report
            ...
            .map(|usage| {
                ...
                format!(
                    "{} | installed:{} auth:{} | plan:{} | remaining:{} | reset:{} | renew:{} | auth_exp:{} | confidence:{:?}",
                    usage.provider,
                    usage.installed,
                    usage.authenticated,
                    plan,
    ...
    pub(super) fn tool_version_info(&self, args: &Value) -> Result<Value, String> {
        ...
        Ok(
            json!({ "content": [{ "type": "text", "text": text }], "current": v.current, "project_type": v.project_type, "last_tag": v.last_tag, "commits_since_tag": v.commits_since_tag }),
        )
    }
    ...
    pub(super) fn tool_version_bump(&self, args: &Value) -> Result<Value, String> {
        ...
        let bump_type = raios_core::core::version::BumpType::parse(level)
        ...
        let r = raios_core::core::version::bump(&path, &bump_type, changelog, tag);
        let text = if r.ok {
            format!(
                "✓ {} → {}  ({})\n{}",
        ...
        Ok(
            json!({ "content": [{ "type": "text", "text": text }], "ok": r.ok, "old_version": r.old_version, "new_version": r.new_version, "changelog_entry": r.changelog_entry }),
        )
    }

    pub(super) fn tool_env_status(&self, args: &Value) -> Result<Value, String> {
        let path = self.resolve_git_path(args)?;
        let r = raios_core::core::env::check(&path);
        let mut lines = vec![format!(
    ...
    pub(super) fn tool_deps_status(&self, args: &Value) -> Result<Value, String> {
        let path = self.resolve_git_path(args)?;
        let r = raios_core::core::deps::check(&path);
    ...
    pub(super) fn tool_run_build(&self, args: &Value) -> Result<Value, String> {
        let path = self.resolve_git_path(args)?;
        let r = raios_core::core::build::build(&path);
        ...
        Ok(
            json!({ "content": [{ "type": "text", "text": text }], "ok": r.ok, "warnings": r.warnings, "errors": r.errors, "duration_ms": r.duration_ms, "project_type": r.project_type }),
        )
    }

    pub(super) fn tool_run_tests(&self, args: &Value) -> Result<Value, String> {
        let path = self.resolve_git_path(args)?;
        let r = raios_core::core::build::test(&path);
