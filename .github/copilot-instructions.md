...
<!-- Updated: 2026-07-01T19:20:10.277Z -->
...
h2 deps
h2 .tmp
h3 .tmp/hf-hub-0.3.2/src/api/sync.rs
h3 .tmp/hf-hub-0.3.2/src/api/tokio.rs
h2 src
...
h3 src/app/ipc.rs
h3 src/app/state.rs
h3 src/app/events/bg_messages.rs
h3 src/ui/panels/content.rs
h3 src/app/events/keyboard/dashboard.rs
```
...
### src/security/verify_chain.rs
```
pub fn record_audit_event(conn: &Connection, event_type: &str, actor: &str, data: &str,) → Result<()>
pub fn record_tool_decision(conn: &Connection, tool: &str, args_hash: &str, matched_rule: &str, event_type: &str, actor: &str,) → Result<()>
pub fn verify_chain(conn: &Connection) → Result<usize>
```

### src/security/secret_scan.rs
```
...
pub fn check_fs_capability(caps: &ToolCapabilities, resolved_target: &std::path::Path, blocked_paths: &[String],) → Result<(), String>
...
### src/security/policy_suggest.rs
```
pub struct PolicySuggestion
pub struct ReviewNote
pub struct PolicySuggestions
pub fn suggest_policy_rules(conn: &Connection, existing: Option<&PolicyConfig>, min_count: usize,) → Result<PolicySuggestions>
...
### src/app/events/keyboard/health.rs
```
impl App
```
...
pub fn cp_session_end_with_summary(conn: &Connection, task_id: &str, run_id: &str, success: bool, summary: Option<&str>,) → Result<()>
